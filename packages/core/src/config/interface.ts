...
import { ITiptapEditor } from '../editor/interface';
import { IMenuGroup } from '../menus/interface';

interface IHoverbarConf {
  // key 即 element type
  [key: string]: {
    // match?: (editor: ITiptapEditor, n: Node) => boolean; // 自定义匹配函数，优先级高于“key 即 element type”
    menuKeys: string[];
  };
}
...
export interface IEditorConfig {
  ...
  onCreated?: (editor: ITiptapEditor) => void;
  onChange?: (editor: ITiptapEditor) => void;
  onDestroyed?: (editor: ITiptapEditor) => void;

  onMaxLength?: (editor: ITiptapEditor) => void;
  onFocus?: (editor: ITiptapEditor) => void;
  onBlur?: (editor: ITiptapEditor) => void;

  /**
   * 自定义粘贴。返回 true 则继续粘贴，返回 false 则自行实现粘贴，阻止默认粘贴
   */
  customPaste?: (editor: ITiptapEditor, e: ClipboardEvent) => boolean;
