...
import { ImageExt, ImageOptions } from '../extensions/ImageExt.ts';
...
// import { createLinkBubbleMenu, createImageBubbleMenu } from '../core/getBubbleMenus.ts';

// import { createTextSelectionBubbleMenu } from '../components/bubbleToolbar/TextSelectionBubbleMenu.ts';
...
export const GetExtionsKit = (options: ExtensionKitOptions) => {
  const extensions = [];
  ...
  if (options.image !== false) {
    extensions.push(
      ImageExt.configure({
        ...
        uploader: options.image?.uploader,
