  ...
  getSvgFiles(entry)
    .then(res => {
      const icon = res.map(val => {
        ...
        return `<imy-icon-${str}></imy-icon-${str}>\n`
      }).join('')

      writeFile(`${outDir}/imy-icon.md`, icon, err => {
        if (err) {
          throw err;
        }
      });
  ...
  getSvgFiles(entry)
    ...
    .then(async res => {
      ...
      await Promise.allSettled(res).then(arr => {
        names = arr.map(c => {
          if (c.value) {
            const template = renderCode(`imy-icon-${c.value.name}`, c.value.content);
            writeFile(`${outDir}/components/imy-icon/${c.value.name}.tsx`, template, err => {
              if (err) {
                throw err;
              }
            });
          ...
          return {
            ...
            compName: `imy-icon-${c.value?.name}`,
          };
        });
      });
      return names;
    ...
    // .then(res => {
    //   const importStr = res.map(c => `import ${c.className} from './components/imy-icon/${c.name}.js';`).join('\n');
...
// 生成所有组件标签，markdown 文件
// swcIconTemplate(entryDir, outputDir);
