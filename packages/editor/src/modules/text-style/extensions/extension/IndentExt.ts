...
export const IndentExtension = Extension.create<IndentOptions>({
  ...
  addCommands() {
    ...
    const applyIndent: (direction: number) => () => Command = (direction) => () => (editor) => {
      const { tr, state, dispatch } = editor
      const { selection } = state
      let trs = tr.setSelection(selection)
      trs = updateIndentLevel(tr, direction)
